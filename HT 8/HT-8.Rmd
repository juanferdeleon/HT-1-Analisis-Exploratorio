...
modeloCaret2 <- train(tipoDeCasa~., data=train, method="nnet",preProcess=c("scale","center"), na.action = na.omit,linout = TRUE)
modeloCaret2
```
...
test$prediccionCaret <- predict(modeloCaret2, newdata = test)
...
#test$predNeuralNet<-round(predi#ct(modelo.nn, newdata = #test[,1:37]),digits =  0)
#u <- union(test$predNeuralNet,t#est$tipoDeCasa)
#t <- table(factor(test$predNeur#alNet, u), #factor(test$tipoDeCasa, u))
